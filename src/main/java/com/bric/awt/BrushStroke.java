 ...
 * @(#)BrushStroke.java
 *
 * $Date: 2015-02-09 09:09:17 +0100 (Mo, 09 Feb 2015) $
 *
...
public class BrushStroke implements Stroke {
	...
	public BrushStroke(float width,float thickness,float theta,long randomSeed) {
		...
		for(int a = 0; a<strokes.length; a++) {
			strokes[a] = new BasicStroke(((float)a+1)*width/(layers),BasicStroke.CAP_BUTT,BasicStroke.JOIN_BEVEL,10);
		}
	}
		...
		for(int a = 0; a<layers; a++) {
			...
			for(int b = 0; b<measuredLayers.length; b++) {
				...
				while(x<measuredLayers[b].getOriginalDistance()) {
					...
					if(streakDistance>0) {
						GeneralPath p2 = measuredLayers[b].getShape(x/measuredLayers[b].getClosedDistance(),
								streakDistance/measuredLayers[b].getClosedDistance());
